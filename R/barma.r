#' Model fit function
#'
#' Funcao para ajuste do modelo BARMA com funcao de ligacao arbitraria (aranda, logit, probit, cloglog)
#'
#' @param y Time series
#' @param ar Autoregressive order
#' @param ma Moving-average order
#' @param link Link function
#' @param h Lags for forecast
#' @param X Covariates
#' @param X_hat Observations of covariates used in forecast
#' @param resid Residuals type
#' @param diag Output
#'
#' @export

# diag = 0 : no graphs/prints (usefull for simulations).
# diag = 1 : plot graphs on screen
# diag = 2 : plot graphs on screen and in pdf file
#
# h : steps forward to make prediction
fit.barma <- function (y, ar=NA, ma=NA, link = "aoz", diag=1,h=6,X=NA,X_hat=NA,resid=1,lambda=1)
{
  #source("barma.fit.r")
  ...
  {
    freq<-frequency(y)
  }else stop("data must be a time-series object")
  ...
  if(linktemp == "aoz"){
    ...
    # fit1 <- barma.fit.aoz(y, ar, ma, link1, names_phi, names_theta, names_beta, diag, h, X, X_hat,resid=resid,lambda=lambda) # lambda?
    invisible(capture.output(fit <- barma.fit.aoz(y, ar, ma, link1, names_phi, names_theta, names_beta, diag, h, X, X_hat,resid=resid,lambda=1))) # lambda?
    loglikmin<-fit$loglik
    myseq<-c(.5,1.5,3,5)
    for(lzera in myseq){
      invisible(capture.output(xii<-barma.fit.aoz(y, ar, ma, link1, names_phi, names_theta, names_beta, diag=0, h, X, X_hat,resid=resid,lambda=lzera)))
      print(paste("Lambda =",lzera));print(paste("Log-Likelihood =",xii$loglik));cat("\n")

      if(loglikmin<xii$loglik) # melhor modelo segundo o loglik
      {
      ...
      barma.fit<- function (y, ar, ma, link, names_phi,names_theta,names_beta,diag,h1,X,X_hat,resid)
      {
        ...
        if(any(is.na(ar)==F) && any(is.na(ma)==F) && any(is.na(X)==T))
        {
          ...
          escore <- function(z)
          {
            ...
            Uphi <-   prec * t(P) %*% mT %*% (ystar-mustar)
            Utheta <- prec * t(R) %*% mT %*% (ystar-mustar)
            Uprec <-  sum(mu * (ystar-mustar) + log(1-y1)
                          - digamma((1 - mu) * prec) + digamma(prec) )
          ...
          opt <- optim(reg, loglik, escore, method = "BFGS",
                       control = list(fnscale = -1, maxit = maxit1, reltol = 1e-9))
        ...
        if(any(is.na(ar)==F) && any(is.na(ma)==F) && any(is.na(X)==F))
        {
          ...
          loglik <- function(z)
          {
            ...
            for(i in (m+1):n)
            {
              ...
              #         print("1sadkljfkasjdfk ")
              eta[i] <- alpha + X[i,]%*%as.matrix(beta) + (phi%*%(ynew[i-ar]-X[i-ar,]%*%as.matrix(beta) )) + (theta%*%error[i-ma])

              #         print("2WRETDGHFGFGJHGJH")
              error[i] <- ynew[i]-eta[i]
              #error[i] <- y[i]-linkinv(eta[i])
              #mui<-linkinv(eta[i])
              #error[i] <- (y[i]-mui)/sqrt((mui*(1-mui))/(1+prec)) # standardized
        ...
        resid2<- (errorhat[(m+1):n])/sqrt(muhat*(1-muhat)/(1+prec))
        z$resid2 <- resid2

        ystar <- log(y1/(1-y1))
        mustar <- digamma(muhat * prec) - digamma((1 - muhat) * prec)
        z$resid3 <- ((ystar-mustar)/sqrt( trigamma(muhat*prec)+trigamma((1-muhat)*prec) ) )

        z$resid4 <- qnorm(pbeta(y1, muhat*prec, (1-muhat)*prec))
        ...
        if(diag>0)
        {
          ...
          par(mar=c(2.8, 2.7, 1.2, 1)) # margens c(baixo,esq,cima,direia)
          par(mgp=c(1.7, 0.45, 0))
          plot(resid2,main=" ",xlab="índices",ylab="resíduos", pch = "+",ylim=c(-4,4))
          lines(t,rep(-3,n+12),lty=2,col=1)
          lines(t,rep(3,n+12),lty=2,col=1)
          lines(t,rep(-2,n+12),lty=3,col=1)
          lines(t,rep(2,n+12),lty=3,col=1)
          ...
          acf(resid2,ylab="FAC",xlab="defasagem") # função de autocorrelação

          pacf(resid2,ylab="FACP",xlab="defasagem") # função de autocorrelação parcial
...
# fit<-barma(y, ar=1, diag=0,link = "aoz", X=NA, X_hat=NA,lambda=0.1)
# loglikmin<-fit$loglik
# myseq<-c(.8,1.5,3,5)
# for(lzera in myseq){
